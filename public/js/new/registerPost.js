...
window.addEventListener("load", async () => {
  ...
  mapIconControll.addEventListener("change", () => {
  ...
  // console.log("dafault categoryFields", categoryFields);
  getAllLocations().then((data) => {
    loading.style.display = "none";
    // console.log(data);

    const cityChoises = new Choices(citySelectBox);
    const neighborhoodChoises = new Choices(neighborhoodSelectBox);
    ...
    const choices = [
      ...
      ...tehranNeighborhoods.map((neighborhood) => ({
        value: neighborhood.id,
        label: neighborhood.name,
      })),
    ];
    neighborhoodChoises.setChoices(choices, "value", "label", false);

    cityChoises.setChoices(
    ...
    citySelectBox.addEventListener("addItem", (e) => {
      // console.log("E -> ", e);
      neighborhoodChoises.clearStore();
      const neighborhoods = data.neighborhoods.filter(
        (neighborhood) => neighborhood.city_id === e.detail.customProperties.id
      );
      ...
      if (neighborhoods.length) {
        ...
        neighborhoodChoises.setChoices(
          [
            {
              value: 0,
  ...
  registerBtn.addEventListener("click", async (e) => {
    e.preventDefault();
    //validations 2 (static and dynamic fields)
    //dynamic validation
    ...
    if (
      ...
      if (res.status === 201) {
        showSwal("پست در صف انتشار قرار گرفت", "success", "اوکی", () => {
